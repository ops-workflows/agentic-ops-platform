...
export default function ApprovalsPage() {
  const [data, setData] = useState<PlatformApprovals>({
    counts_by_status: {},
    items: [],
    total: 0,
    limit: PAGE_SIZE,
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      const payload = await apiFetch<PlatformApprovals>(
        `/api/platform/approvals?limit=${PAGE_SIZE}&offset=${offset}`,
      );
      setData(payload);
    } catch (err) {
            ...
            <table className="w-full min-w-[980px]">
              ...
              <tbody>
                {data.items.map((item) => (
                  <tr
                    key={item.id}
                    className="border-b border-ops-border/50 text-sm align-top last:border-0"
                  >
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`rounded-full px-2.5 py-1 text-[10px] uppercase tracking-[0.14em] ${tones[status] || 'bg-ops-surface-raised text-[var(--color-text-tertiary)]'}`}
    >
      {status}
    </span>
  );
