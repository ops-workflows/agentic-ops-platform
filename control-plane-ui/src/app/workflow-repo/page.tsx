...
export default function WorkflowRepoPage() {
  ...
  async function handleSync() {
    ...
    setError(null);
    try {
      const payload = await apiFetch<WorkflowRepoStatus>(
        '/api/platform/workflow-repo/sync',
        { method: 'POST' },
      );
      setStatus(payload);
    } catch (err) {
  ...
  async function handlePin() {
    ...
    try {
      const payload = await apiFetch<WorkflowRepoStatus>(
        '/api/platform/workflow-repo/pin',
        {
          method: 'POST',
          ...
          <article className="rounded-card border border-ops-border bg-ops-surface p-5">
            ...
            {status.source_mode === 'remote' ? (
              ...
              <p className="mt-4 text-sm text-[var(--color-text-tertiary)]">
                No remote workflow repo configured — running from a local
                checkout.
              </p>
            )}
          </article>
