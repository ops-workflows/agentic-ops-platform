  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-[11px] uppercase tracking-[0.2em] text-[var(--color-text-tertiary)]">Connectors</p>
      ...
      {loading ? <p className="text-sm text-[var(--color-text-tertiary)]">Loading connectors...</p> : null}
      {error ? <p className="text-sm text-[var(--color-error)]">{error}</p> : null}

      {!loading && !error ? (
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          {connectors.map((connector) => (
            <article key={connector.id} className="rounded-card border border-ops-border bg-ops-surface p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h2 className="text-lg font-medium text-[var(--color-text-primary)]">{connector.name}</h2>
                  <p className="mt-1 text-sm leading-6 text-[var(--color-text-secondary)]">{connector.summary}</p>
                </div>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                ...
                <Meta label="Target workflow" value={connector.target_workflow || 'Unassigned'} />
                <Meta label="Target channel" value={connector.target_channel || '-'} />
              ...
              {connector.tags.length > 0 ? (
                <div className="mt-4 flex flex-wrap gap-2">
                  {connector.tags.map((tag) => (
                    <span key={tag} className="rounded-full bg-[var(--color-info-muted)] px-2.5 py-1 text-[10px] uppercase tracking-[0.12em] text-[var(--color-info)]">
                      {tag}
                    </span>
                  ))}
                </div>
          ...
          {connectors.length === 0 ? (
            <p className="text-sm text-[var(--color-text-tertiary)]">No connectors found.</p>
          ) : null}
        </div>
      ) : null}
    </div>
