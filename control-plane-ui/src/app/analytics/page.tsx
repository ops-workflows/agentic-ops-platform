...
import { apiFetch, type Analytics } from '@/lib/api';
...
export default function AnalyticsPage() {
  ...
  const fetchForMonth = useCallback((month: Date) => {
    ...
    apiFetch<Analytics>(
      `/api/analytics?start_date=${startStr}&end_date=${endStr}`,
    )
      .then(setAnalytics)
      .catch(console.error)
  ...
  if (loading && !analytics)
    return (
      <p className="text-[var(--color-text-tertiary)]">Loading analytics...</p>
    );
  if (!analytics)
    return (
  ...
  const successRate =
    analytics.total_tasks > 0
      ? (analytics.succeeded / analytics.total_tasks) * 100
      : 0;
  ...
  const maxDailyCount = Math.max(
    ...analytics.daily_counts.map((item) => item.count),
    1,
  );
  ...
  const tokensByWorkflow = analytics.tokens_by_workflow
    ? Object.entries(analytics.tokens_by_workflow)
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        ...
        <div className="bg-ops-surface border border-ops-border rounded-card p-5">
          ...
          <p className="text-3xl font-medium mt-2 text-[var(--color-text-primary)]">
            {analytics.avg_duration_sec !== null
              ? `${Math.round(analytics.avg_duration_sec)}s`
              : 'N/A'}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-ops-surface border border-ops-border rounded-card p-6">
          <h3 className="text-base font-medium mb-5 text-[var(--color-text-primary)]">
          ...
          {hasTokenData ? (
              ...
              {tokensByWorkflow.map((item) => (
                <div key={item.workflow}>
                  <div className="flex justify-between text-sm mb-1.5">
                    <span className="text-[var(--color-text-secondary)] truncate mr-3">
                      {item.workflow}
                    </span>
            ...
            <p className="text-[var(--color-text-tertiary)] text-sm">
              Token breakdown not available yet. Ensure backend returns{' '}
              <code className="text-xs">tokens_by_workflow</code>.
            </p>
          )}
        </div>
              ...
              <div key={item.workflow}>
                ...
                <div className="w-full bg-ops-border rounded-full h-1.5">
                  <div
                    ...
                    style={{
                      width: `${(item.count / maxWorkflowCount) * 100}%`,
                    }}
                  />
                </div>
              </div>
        ...
        <div className="bg-ops-surface border border-ops-border rounded-card p-6">
          ...
          <div className="space-y-4">
            {statuses.map((item) => {
              const pct =
                analytics.total_tasks > 0
                  ? (item.count / analytics.total_tasks) * 100
                  : 0;
              const color =
                {
